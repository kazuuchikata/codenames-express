...
function onNewWebsocketConnection(socket) {
    ...
    socket.on("disconnect", () => {
        ...
        socket.broadcast.emit("user_count", onlineClients.size);
    });

    // echoes on the terminal every "hello" message this socket sends
    socket.on("hello", helloMsg => {
        console.info(`Socket ${socket.id} says: "${helloMsg}"`);
        socket.emit("user_count", onlineClients.size);
        socket.broadcast.emit("user_count", onlineClients.size); // this will broadcast the user count to all other sockets connected
    });
}
